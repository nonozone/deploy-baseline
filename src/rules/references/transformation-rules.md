...
- generate from the bundled baseline skeleton
- adapt placeholders to the detected stack, mode, and database

### Lightweight existing project

- preserve project-specific logic or values only when they still matter
- prefer baseline names and paths when reorganizing the structure
- fill missing baseline pieces and migrate useful values forward
...
- prefer baseline directory convergence over keeping historical layout drift
...
- Use `deploy/env/app.env.example` as the single canonical env example source.
- Use `deploy/env/app.dev.env` as the standard local-development env path.
- Use `deploy/env/app.prod.env` as the standard production env path.
- Keep database, provider, and fuller runtime variables grouped inside `deploy/env/app.env.example`.
...
- If a legacy root `.env` exists and `deploy/env/app.dev.env` does not, copy it once during setup with an explicit migration notice.
