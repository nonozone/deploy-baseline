...
- Verify `deploy/env/app.prod.env` is not missing active keys that now exist in `deploy/env/app.env.example`.
- Verify `deploy/env/app.dev.env` is aligned with `deploy/env/app.env.example`, or explicitly documented as intentionally absent.
