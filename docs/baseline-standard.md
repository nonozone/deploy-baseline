...
- 新增 `.env` 分层或新的部署 env 文件时，也必须先检查项目现有 env 配置并做迁移，不能直接用模板覆盖旧 env 文件
...
- 明确占位规则：敏感值默认留空，镜像版本占位用 `replace-with-git-sha`，非敏感默认值可保留可运行示例，外部非敏感示例可使用 `example.com` 风格地址
- 对状态型服务和验证相关变量单独分组
- 如果项目已存在 `.env`、`.env.local`、`deploy/env/*.env` 等真实配置文件，引入新 env 方案时必须先检查现状并迁移有效值；只允许非破坏性补齐缺失项，不允许直接覆盖原配置
...
- 生产镜像建议优先使用 Git Commit SHA 或语义化版本号作为 tag
